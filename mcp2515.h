...
class MCP2515 {
 public:
  enum ERROR { ERROR_OK = 0, ERROR_FAIL = 1, ERROR_ALLTXBUSY = 2, ERROR_FAILINIT = 3, ERROR_FAILTX = 4, ERROR_NOMSG = 5 };
 ...
 private:
  ...
  enum /*class*/ TXBnCTRL : uint8_t { TXB_ABTF = 0x40, TXB_MLOA = 0x20, TXB_TXERR = 0x10, TXB_TXREQ = 0x08, TXB_TXIE = 0x04, TXB_TXP = 0x03 };
 ...
 public:
  ...
  ERROR sendMessage(const TXBn txbn, const can_message_t *frame);
  ERROR sendMessage(const can_message_t *frame);
  ERROR readMessage(const RXBn rxbn, can_message_t *frame);
  ERROR readMessage(can_message_t *frame);
